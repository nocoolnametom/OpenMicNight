...
class episodeassignmentActions extends autoEpisodeassignmentActions
{
    ...
    public function executeFuture(sfWebRequest $request)
    {
        ...
        $q = Doctrine_Query::create()
                ...
                ->leftJoin('EpisodeAssignment.Episode Episode ON (Episode.id = EpisodeAssignment.episode_id)')
                ->leftJoin('Deadline ON (Deadline.author_type_id = EpisodeAssignment.author_type_id AND Deadline.subreddit_id = Episode.subreddit_id)')
                //->where('Episode.id = EpisodeAssignment.episode_id')
                ->where('EpisodeAssignment.missed_deadline <> 1');
        if (array_key_exists('sf_guard_user_id', $params))
